...
<div class="main-content">
    ...
    {{-- ── REPORT ROW: Sentiment + Coupon Stats ───────────────────────────── --}}
        ...
        <div class="col-xl-7">
            <div class="section-card">
                <div class="section-title">
                    ...
                    <small class="text-muted ms-auto">Berdasarkan review antara {{ $startDate->format('d M Y') }} – {{ $endDate->format('d M Y') }}</small>
                </div>
                <div class="d-flex gap-3 align-items-center">
                    <div style="width:320px;">
                        <canvas id="sentimentChart" width="320" height="240"></canvas>
                    </div>
                    <div class="flex-grow-1">
                        <div style="display:flex;gap:1rem;align-items:center">
                            <div style="flex:1">
                                <div class="fw-semibold">Positif (4–5)</div>
                                <div class="fs-4 text-success">{{ number_format($sentiment->positive) }}</div>
                            </div>
                            <div style="flex:1">
                                <div class="fw-semibold">Netral (3)</div>
                                <div class="fs-4 text-warning">{{ number_format($sentiment->neutral) }}</div>
                            </div>
                            <div style="flex:1">
                                <div class="fw-semibold">Negatif (1–2)</div>
                                <div class="fs-4 text-danger">{{ number_format($sentiment->negative) }}</div>
                            </div>
                        </div>
                        <small class="text-muted">Visualisasi menggunakan Query Builder; membantu melihat persebaran rating.</small>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        <div class="card-stat text-center">
                            <div class="icon" style="background:#e8f4fd;color:#1a6fad"> <i class="bi bi-check-circle"></i> </div>
                            <div class="value">{{ number_format($couponStats['active']) }}</div>
                            <div class="label">Kupon Aktif</div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card-stat text-center">
                            <div class="icon" style="background:#fff0f0;color:#c0392b"> <i class="bi bi-exclamation-circle"></i> </div>
                            <div class="value">{{ number_format($couponStats['expired']) }}</div>
                            <div class="label">Kupon Kadaluarsa</div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card-stat text-center">
                            <div class="icon" style="background:#f7f0ff;color:#8e44ad"> <i class="bi bi-bar-chart-line-fill"></i> </div>
                            <div class="value">{{ number_format($couponStats['used_total']) }}</div>
                            <div class="label">Total Penggunaan</div>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    try {
        const _sentiment = @json($sentiment);
        ...
        if (ctx) {
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [_sentiment.positive, _sentiment.neutral, _sentiment.negative],
        ...
        console.error('Sentiment chart init error', e);
    }
