...
class AdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        // ── Analisis Sentimen (Query Builder, berdasarkan reviews.rating) ─────
        $sentimentQuery = DB::table('reviews')
            ->whereBetween('created_at', [$startDate, $endDate]);

        $sentiment = $sentimentQuery->select(
        ...
        $sentiment = (object) [
            'positive' => (int) ($sentiment->positive ?? 0),
            'neutral'  => (int) ($sentiment->neutral ?? 0),
            'negative' => (int) ($sentiment->negative ?? 0),
        ];

        // ── Coupon stats (Query Builder) ─────────────────────────────────────
        $now = now();

        $totalCouponsActive = DB::table('coupons')
        ...
        $totalCouponsExpired = DB::table('coupons')
            ->whereNotNull('expires_at')
            ->where('expires_at', '<=', $now)
            ->count();

        $totalCouponUsage = (int) DB::table('coupons')->sum('used_count');

        // pass to view
        $couponStats = [
            'active' => $totalCouponsActive,
            'expired' => $totalCouponsExpired,
            'used_total' => $totalCouponUsage,
        ];
